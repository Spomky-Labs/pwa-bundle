...
return static function (DefinitionConfigurator $definition): void {
    $definition->rootNode()
        ...
        ->defaultValue('/idb')
        ->info('The public path to the local IndexDB. Only used if use_cdn is false.')
        ->end()
        ...
        ->defaultFalse()
        ->info('Whether to enable navigation preload. This speeds up navigation requests by making the network request in parallel with service worker boot-up. Note: Do not enable if you are precaching HTML pages (e.g., with offline_fallback or warm_cache_urls), as it would be redundant.')
        ->end()
        ...
        ->info('The name of the offline cache.')
        ...
        ->thenInvalid(
            'Invalid caching strategy "%s". Should be one of: ' . implode(', ', CacheStrategyInterface::STRATEGIES)
        )
        ->end()
        ->end()
        ->scalarNode('max_entries')
        ->defaultNull()
        ->info('The maximum number of entries in the cache (for "CacheFirst" and "NetworkFirst" strategy only).')
        ->end()
        ->scalarNode('max_age')
        ->defaultNull()
        ->info(
        ...
        ->defaultFalse()
        ->info('Whether to support range requests (for "CacheFirst" strategy only).')
        ->end()
        ...
        ->integerPrototype()
        ->end()
        ->info('The cacheable response statuses. if set to [200], only 200 status will be cached.')
        ->end()
